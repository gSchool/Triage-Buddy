...
- **`src/triage_buddy/adapters/`**
  ...
  - `llm/zai.py`: `ZaiProvider` — Z.ai (Zhipu AI) GLM (`glm-4.6`) via `zai-sdk`, JSON mode, `temperature=0`. Per-request `timeout` on the client (SDK's own `max_retries=0` — this adapter owns retries). Optional `[zai]` extra; SDK imported lazily. `ZAI_API_KEY`. API failures → `LLMError`.
...
4. Determine the ports and adapters. ✅ LLM port; driven adapters: `mock`, `groq` (Llama), `gemini`, `zai` (GLM). Driving adapters: CLI, web (form + JSON API).
